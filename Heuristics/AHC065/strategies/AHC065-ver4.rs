...
fn precompute_dist(n: usize, exit_r: usize, exit_c: usize, belts: &[Belt]) -> Vec<usize> {
...
fn get_best_move(
    ...
    for i in 0..6 {
        if target + i < 400 {
            ...
            if !active_belts.contains(&h) {
                active_belts.push(h);
            }
            if !active_belts.contains(&v) {
                active_belts.push(v);
            }
        }
    }
        ...
        for node in &beam {
            ...
            for i in 0..6 {
                if node.state.next_to_remove + i < 400 {
                    ...
                    let h = (r / 2) as usize;
                    let v = 10 + (c / 2) as usize;
                    if !node_active.contains(&h) {
                        node_active.push(h);
                    }
                    if !node_active.contains(&v) {
                        node_active.push(v);
                    }
                }
            }
...
fn main() {
    ...
    while current_state.next_to_remove < n * n
        && current_state.grid[10] == current_state.next_to_remove as u16
    {
    ...
    while current_state.next_to_remove < n * n {
        ...
        let best_first_op = get_best_move(&current_state, &belts, &dist, beam_width, max_depth);
