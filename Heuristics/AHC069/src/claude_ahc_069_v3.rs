...
fn boundary_segments(
    cells: &[(usize, usize)],
) -> (Vec<(isize, usize, usize)>, Vec<(isize, usize, usize)>) {
...
fn dead_cells(board: &Board, cells: &[(usize, usize)], sc: &mut Scratch) -> usize {
    ...
    for r in 0..N {
        for c in 0..N {
            if board.blocked[r][c] != 0 || sc.mark[r][c] == g || sc.vis[r][c] == g {
                continue;
            }
            sc.vis[r][c] = g;
            sc.stack.clear();
            ...
            while let Some((ar, ac)) = sc.stack.pop() {
                ...
                for (nr, nc) in neighbours(ar, ac) {
                    if board.blocked[nr][nc] == 0 && sc.mark[nr][nc] != g && sc.vis[nr][nc] != g {
                        sc.vis[nr][nc] = g;
                        sc.stack.push((nr, nc));
                    }
                }
...
fn grow_region(
    ...
    while region.len() < p {
        ...
        let cnt = neighbours(r, c)
            .filter(|&(nr, nc)| inside(sc, nr, nc))
            .count() as u8;
        if cnt != stored {
            heap.push((cnt, r, c));
            continue;
        ...
        for (nr, nc) in neighbours(r, c) {
            if board.blocked[nr][nc] == 0 && !inside(sc, nr, nc) {
                let k = neighbours(nr, nc)
                    .filter(|&(ar, ac)| inside(sc, ar, ac))
                    .count() as u8;
                heap.push((k, nr, nc));
            }
        }
    ...
    for idx in 0..m {
        ...
        if chosen.is_none() && use_fallback && in_budget {
            ...
            for &(_, r, c) in &seeds {
                ...
                if used
                    .iter()
                    .any(|&(ur, uc)| ur.abs_diff(r) < 4 && uc.abs_diff(c) < 4)
                {
                    continue;
                }
                used.push((r, c));
