// AHC069 - v4: v3 + paid relocation of active groups.
//
// Moving group j costs max(round(V_j * R), 1), and since C_j is the *minimum*
// compactness over every position it ever held, the recorded quantity is really the
// *maximum perimeter* it ever occupied. Two consequences drive v4:
//
//   * a group may be relocated into any shape whose perimeter is <= that record with
//     no loss of fee at all - in particular a fallback-grown snake (huge perimeter)
//     can later be repaired into a compact square for free;
//   * the cost scales with V_j, so shoving *cheap* groups aside to seat an expensive
//     one is nearly free, dramatically so at R = 0.001.
//
// So when the arriving group cannot be placed in its most compact shape, v4 looks for
// an anchor where that shape *would* fit if a few cheap groups moved out of the way,
// relocates them, and takes the deal whenever
//     V_i * C_best  -  sum of move costs  >  value of the best no-move placement.
//
// Everything below this is v3, unchanged:
//
...
// ---- relocation knobs (new in v4) -------------------------------------------
const MOVES: f64 = 1.0; // > 0 enables relocation; 0 reproduces v3 exactly
const MAX_DISPLACE: usize = 3; // most groups shoved aside for one arrival
const MOVE_TOPK: usize = 3; // target anchors examined in full
const MOVE_MARGIN: f64 = 1.0; // required profit over the no-move option, in money
const MOVE_TIME_MS: u128 = 1200; // stop attempting relocation past this point
// Re-seating breadth for displaced groups. 1 means "best contact, no refinement", and
// measurement says that is right: giving displaced groups the arriving group's full
// treatment (time affinity + stranded-cell penalty, topk 8) scored 36W/44L, -0.15% over
// 100 seeds. The refinement chases layout quality for a group that is only passing
// through, and the tighter contact-only choice packs better.
...
struct Board {
    ...
    pond_pre: [[u32; N + 1]; N + 1], // 2D prefix sum of `pond` (static)
    cost: [[f64; N]; N], // per-cell share of the occupant's move cost
...
impl Board {
    ...
    /// `share` is the occupant's move cost spread over its cells, so summing `cost`
    /// across a whole region gives back exactly that group's move cost.
    ...
    /// Ponds inside the shape at this anchor (a target region must have none) and the
    /// move cost of the groups sitting in the way, pro-rated by overlap.
...
struct Active {
    t: i64,
    p: usize,
    /// Largest perimeter this group has ever occupied - that is what fixes its fee,
    /// so any relocation must use a shape at or below it.
    perim: usize,
    move_cost: i64,
    cells: Vec<(usize, usize)>,
}

/// Weights shared by every placement decision.
struct Weights {
    wall: f64,
    pond: f64,
    occ: f64,
...
/// Best anchor for a group whose shape must not exceed `max_perim`, using the same
/// two-stage scoring as an arriving group: perimeter classes in increasing order, best
/// contact first, then the top `topk` re-ranked by departure-time affinity and stranded
/// free cells. Re-seating a displaced group carelessly is what made relocation a net
/// loss on some seeds, so it gets the full treatment too.
fn scored_placement(
    board: &Board,
    shapes: &[Shape],
    max_perim: usize,
...
fn main() {
    ...
    // groups are indexed by their arrival number so `owner` maps straight into them
    ...
    // R is given with exactly 3 decimals, so the move cost max(round(V*R), 1) is an
    // exact rational. Computing it in integers avoids the off-by-one that f64 gives
    // whenever V*R lands on a half (verified against the tester on seed 0).
    ...
    let mut relocations = 0usize; // arrivals that triggered a relocation
    let mut moved_groups = 0usize;
    let mut move_spend: i64 = 0;
    let mut dep: Vec<i64> = vec![0; m];
    let mut cands: Vec<(f64, usize, usize, usize)> = Vec::new();
    let mut targets: Vec<(f64, f64, usize, usize, usize)> = Vec::new(); // (cost, -contact, si, r, c)
    ...
    for idx in 0..m {
        ...
        // booked at placement time: relocations are restricted to shapes at or below
        // the group's recorded perimeter, so its compactness can never get worse.
        ...
        // 3e. relocation: if the no-move option is worse than the most compact shape,
        // look for a target anchor blocked only by groups cheap enough to shove aside.
        let base_value = match chosen.as_ref() {
            Some((_, per)) => v * 4.0 * (p as f64).sqrt() / *per as f64,
            None => 0.0,
        };
        let headroom = v * c_best - base_value; // most that relocating could ever win
        ...
        {
            // target anchors of the most compact class, ranked by how much the groups
            // in the way would cost to move (pro-rated, so this is only an estimate)
            targets.clear();
                ...
                for r in 0..=N - sh.bh {
                    for c in 0..=N - sh.bw {
                        ...
                        if pond != 0 || est <= 0.0 || est >= headroom {
                            continue; // ponds cannot move, and free anchors already lost
                        }
            ...
            for &(_, _, si, r, c) in targets.iter() {
                ...
                // exact set of groups standing in the way
                ...
                // trial: lift the blockers, seat the arrival, then re-seat each blocker
                // (largest first) in any shape at or below its recorded perimeter
                ...
                // undo the trial either way; the plan is applied later, once
        ...
        // 3f. admission decision on the better of the two options, net of move costs
        ...
        let free_pass = u < U_FREE && (t - s) <= FREE_DUR; // near-empty park keeps short stays
        ...
        // 4. emit the moves, then the decision
        ...
        // Moves are simultaneous: every mover vacates before any of them is placed. A
        // mover's new cells may legally overlap another mover's old cells (that is what
        // makes swaps possible), so applying them one at a time would corrupt `owner`.
        for (id, _) in &moves {
            let cells = std::mem::take(&mut groups[*id].as_mut().unwrap().cells);
            board.remove(&cells);
        }
